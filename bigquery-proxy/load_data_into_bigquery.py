...
parser.add_argument("-c", "--catalog-path", help="Path to the annotated catalog JSON file",
                    default="~/code/tandem-repeat-catalogs/results__2026-02-01/release_draft_2026-02-01/TRExplorer.repeat_catalog_v2.hg38.1_to_1000bp_motifs.EH.with_annotations.json.gz")
...
parser.add_argument("--hprc256-tsv", default="../data-prep/hprc_lps.2025_12.per_locus_and_motif.256_samples.tsv.gz")
...
parser.add_argument("--hprc256-purity-and-methylation-tsv",
                    #default="../data-prep/hprc-lps/trgt-hprc.allele_size_purity_and_methylation.tsv.gz",
                    default=None,
                    help="TSV with per-locus allele size, purity, and methylation distributions from TRGT-HPRC")
...
parser.add_argument("--tanudisastro-2024-lookup-json",
                    default="../data-prep/literature_review/Tanudisastro_2024/Tanudisastro_2024_lookup.json.gz",
                    help="JSON lookup with sc-eTR data from Tanudisastro et al. 2024")
...
def population_data_sanity_check(row, dataset_name):
    ...
    if not pd.isna(row.get("unique_allele_lengths")) and (row["unique_allele_lengths"] > row["max_allele"] - row["min_allele"] + 1):
        errors.append((f"{dataset_name}: unique_allele_lengths > max_allele - min_allele + 1", f"{row['locus_id']}: min_allele == {row['min_allele']}, max_allele == {row['max_allele']}, and unique_allele_lengths == {row['unique_allele_lengths']}"))
    ...
    if not pd.isna(row.get("unique_allele_lengths")) and (row["unique_allele_lengths"] < len(allele_set)):
        errors.append((f"{dataset_name}: unique_allele_lengths < len(allele_set)", f"{row['locus_id']}: unique_allele_lengths == {row['unique_allele_lengths']}"))
    if not pd.isna(row.get("unique_allele_lengths")) and (row["num_called_alleles"] < row["unique_allele_lengths"]):
        errors.append((f"{dataset_name}: num_called_alleles < unique_allele_lengths", f"{row['locus_id']}: num_called_alleles == {row['num_called_alleles']}, unique_allele_lengths == {row['unique_allele_lengths']}"))
...
def parse_allele_histograms_from_tsv(tsv_path, dataset_name):
    ...
    for _, row in tqdm.tqdm(df.iterrows(), unit=" loci", unit_scale=True, total=total_records):
        ...
        lookup[locus_id] = {
            ...
            "unique_allele_lengths": int(row["unique_allele_lengths"]),
    ...
    with gzip.open(tsv_path, "rt") as f:
        ...
        for line in tqdm.tqdm(f, unit=" lines", unit_scale=True):
            ...
            lookup[locus_id] = {
                ...
                #"unique_allele_lengths": int(float(fields[col_indices["uniqueLengths"]])) if fields[col_indices["uniqueLengths"]] else None,
...
if args.hprc256_purity_and_methylation_tsv:
    print(f"Parsing HPRC256 purity/methylation data from {args.hprc256_purity_and_methylation_tsv}")
    with gzip.open(os.path.expanduser(args.hprc256_purity_and_methylation_tsv), "rt") as f:
        header = f.readline().strip().split("\t")
        for line in f:
            fields = line.strip().split("\t")
            locus_id = fields[0]
            hprc256_purity_methylation_lookup[locus_id] = {
                "allele_size_and_purity_distribution": fields[1] if len(fields) > 1 else "",
                "allele_size_and_methylation_distribution": fields[2] if len(fields) > 2 else "",
            }
    print(f"Parsed {len(hprc256_purity_methylation_lookup):,d} records from {args.hprc256_purity_and_methylation_tsv}")
...
if hprc256_lookup and aou1027_lookup:
    ...
    for column in "min_allele", "mode_allele", "stdev", "median", "99th_percentile", "max_allele":
        # Filter out keys where either dataset has None for this column
        valid_keys = [k for k in shared_keys
                      if hprc256_lookup[k][column] is not None
                      and aou1027_lookup[k][column] is not None]
        if not valid_keys:
            print(f"WARNING: No valid keys for {column} correlation")
            continue
        pearsonr, _ = scipy.stats.pearsonr(
            [hprc256_lookup[k][column] for k in valid_keys],
            [aou1027_lookup[k][column] for k in valid_keys])
        if pearsonr < 0.66:
...
## Parse Tanudisastro 2024 sc-eTR lookup
tanudisastro_2024_lookup = {}
if args.tanudisastro_2024_lookup_json:
    print(f"Parsing Tanudisastro 2024 lookup from {args.tanudisastro_2024_lookup_json}")
    fopen = gzip.open if args.tanudisastro_2024_lookup_json.endswith('gz') else open
    with fopen(args.tanudisastro_2024_lookup_json, 'rt') as f:
        tanudisastro_2024_lookup = json.load(f)
    print(f"Parsed {len(tanudisastro_2024_lookup):,d} records from Tanudisastro 2024 lookup")

## Parse Manigbas 2024 PheWAS lookup
manigbas_2024_lookup = {}
if args.manigbas_2024_lookup_json:
    print(f"Parsing Manigbas 2024 lookup from {args.manigbas_2024_lookup_json}")
    fopen = gzip.open if args.manigbas_2024_lookup_json.endswith('gz') else open
...
for record_i, record in tqdm.tqdm(enumerate(catalog), unit=" records", unit_scale=True):
    ...
    if record["LocusId"] in hprc256_purity_methylation_lookup:
        ...
        if pm_record["allele_size_and_purity_distribution"]:
            record["HPRC256_AlleleSizeAndPurityDistribution"] = pm_record["allele_size_and_purity_distribution"]
        if pm_record["allele_size_and_methylation_distribution"]:
            record["HPRC256_AlleleSizeAndMethylationDistribution"] = pm_record["allele_size_and_methylation_distribution"]
    ...
    if record["LocusId"] in aou1027_lookup:
        ...
        #record["AoU1027_UniqueAlleleLengths"] = aou1027_record["unique_allele_lengths"] 
    ...
    if record["ReferenceRegion"] in vamos_columns_lookup:
        ...
        if vamos_data.get("VamosUniqueMotifs") is not None:
    ...
    if record["LocusId"] in manigbas_2024_lookup:
        counters["rows_with_manigbas_2024_data"] += 1
        m2024_data = manigbas_2024_lookup[record["LocusId"]]
        record["Manigbas2024_AssociatedTraits"] = m2024_data["AssociatedTraits"]
        record["Manigbas2024_MinPvalue"] = m2024_data["MinPvalue"]
        record["Manigbas2024_Details"] = json.dumps(m2024_data["Details"])

        # Track whether this locus has associations or is a negative result
        if m2024_data["AssociatedTraits"]:
